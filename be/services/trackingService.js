...
const CARRIER_PATTERNS = [
  {
    carrier: "UPS",
    regex: /^1Z[A-Z0-9]{16}$/i,
    label: "UPS",
    trackingUrl: "https://www.ups.com/track?tracknum={code}",
  },
  {
    carrier: "FedEx",
    regex: /^(\d{12}|\d{15}|\d{20}|96\d{20})$/,
    label: "FedEx",
    trackingUrl: "https://www.fedex.com/fedextrack/?trknbr={code}",
  },
  {
    carrier: "USPS",
    regex: /^(9[234]\d{20}|[A-Z]{2}\d{9}[A-Z]{2})$/i,
    label: "USPS",
    trackingUrl: "https://tools.usps.com/go/TrackConfirmAction?tLabels={code}",
  },
  {
    carrier: "DHL",
    regex: /^(\d{10}|[A-Z]{3}\d{10}|JD\d{18})$/i,
    label: "DHL",
  ...
  },
  {
    ...
    regex: /^\d{8,11}$/,
    label: "GLS",
    trackingUrl: "https://gls-group.eu/track/{code}",
  },
  {
    carrier: "DPD",
    regex: /^\d{14}$/,
    label: "DPD",
    trackingUrl: "https://www.dpd.com/tracking?parcelNumber={code}",
  },
  {
    carrier: "InPost",
    regex: /^\d{24}$/,
    label: "InPost",
    trackingUrl: "https://inpost.pl/sledzenie-przesylek?number={code}",
  },
  {
    carrier: "Vegefoods",
    regex: /^VGF-[A-Z0-9]{4}-[A-Z0-9]{6}$/,
    label: "Vegefoods Delivery",
    trackingUrl: null,
  },
];
    ...
    if (p.regex.test(code)) {
      return {
        ...
        trackingUrl: p.trackingUrl
          ? p.trackingUrl.replace("{code}", code)
          : null,
      };
    }
  }
...
const assignTracking = async (
  orderId,
  { trackingCode, carrierCode, estimatedDelivery, note } = {},
) => {
  ...
  if (order.userEmail) {
    setImmediate(() =>
      sendTrackingAssignedEmail(order, code, detected, estimatedDelivery).catch(
        (err) =>
          console.error(
            "[trackingService] tracking email failed:",
            err.message,
          ),
      ),
    );
...
const addTrackingEvent = async (
  orderId,
  { status, location, note, actor = "system", timestamp } = {},
) => {
  const ref = db.collection(ORDERS_COL).doc(orderId);
  const snap = await ref.get();
  if (!snap.exists) throw new AppError("Order not found.", 404);
...
const ingestWebhook = async (carrier, payload) => {
  ...
  switch (carrierUpper) {
    case "UPS":
      normalized = {
        ...
        location: [
          payload.ActivityLocation?.City,
          payload.ActivityLocation?.CountryCode,
        ]
          .filter(Boolean)
          .join(", "),
  ...
  if (snap.empty) {
    console.warn(
      `[trackingService] Webhook: no order for tracking# ${normalized.trackingNumber}`,
    );
...
const sendTrackingAssignedEmail = async (
  order,
  trackingCode,
  carrierInfo,
  estimatedDelivery,
