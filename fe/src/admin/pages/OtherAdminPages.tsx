...
import AdminDropdown, { StatusDropdown } from "../components/AdminDropdown";
...
`;
// ── USERS PAGE ──────────────────────────────────────────────────────────
const userStatusV = (s: string) =>
  s === "active" ? "success" : s === "banned" ? "danger" : "neutral";
const roleV = (r: string) =>
  r === "admin" ? "info" : r === "editor" ? "warning" : "neutral";
...
export const UsersPage: React.FC = () => {
  ...
  const cycleBan = useCallback(
    async (u: AdminUser) => {
      ...
      try {
        ...
        dispatch(
          showAdminToast({
            message: `${u.name} ${next === "banned" ? "banned" : "reactivated"}`,
            type: next === "banned" ? "warning" : "success",
          }),
        );
        refetch();
      } catch (err) {
  ...
  const handleBulkAction = useCallback(
    async (action: "active" | "inactive" | "banned" | "delete") => {
      ...
      try {
        if (action === "delete") {
          ...
          dispatch(
            showAdminToast({
              message: `${ids.length} user(s) set to ${action}`,
              type: "success",
            }),
          );
        }
  ...
  return (
    <>
          ...
          <TR key={u.uid}>
            ...
            <TD>
              <PersonCell>
                ...
                <div>
                  <PersonName>{u.name}</PersonName>
                  <PersonSub>{u.email}</PersonSub>
                </div>
              </PersonCell>
            </TD>
            <TD>
              <StatusPill $variant={roleV(u.role) as any}>{u.role}</StatusPill>
            </TD>
            <TD>
              <StatusPill $variant={userStatusV(u.status) as any}>
                {u.status}
              </StatusPill>
            </TD>
          ...
          <ModalBox $width="520px" onClick={(e) => e.stopPropagation()}>
            ...
            <ModalBody>
              <AdminFlex as="div" $gap="16px" style={{ marginBottom: 20 }}>
                <Avatar style={{ width: 52, height: 52, fontSize: "1rem" }}>
                  {selected.name
                    .split(" ")
                    .map((n: string) => n[0])
                    .join("")
                ...
                <div>
                  ...
                  <div
                    style={{ color: t.colors.textMuted, fontSize: "0.875rem" }}
                  >
                    {selected.email}
                  </div>
                  <AdminFlex as="div" $gap="8px" style={{ marginTop: 6 }}>
                    <StatusPill $variant={roleV(selected.role) as any}>
                      {selected.role}
                    </StatusPill>
                    <StatusPill $variant={userStatusV(selected.status) as any}>
                      {selected.status}
                    </StatusPill>
                  </AdminFlex>
                </div>
            ...
            <ModalFooter>
              <AdminBtn $variant="ghost" onClick={() => setSelected(null)}>
                Close
              </AdminBtn>
            </ModalFooter>
          </ModalBox>
      ...
      {deleteId &&
        (() => {
          const u = (users ?? []).find((x) => x.uid === deleteId);
          if (!u) return null;
          return (
            <ModalBackdrop onClick={() => setDeleteId(null)}>
              <ModalBox $width="400px" onClick={(e) => e.stopPropagation()}>
                ...
                <ModalBody>
                  ...
                  >
                    Delete <strong>{u.name}</strong>? This action is permanent.
                  </p>
                </ModalBody>
                <ModalFooter>
          ...
          <ModalBox $width="420px" onClick={(e) => e.stopPropagation()}>
            ...
            <ModalBody>
              ...
              >
                {bulkConfirm === "delete" ? (
                  <>
                    Permanently delete <strong>{selIds.size} user(s)</strong>?
                    This cannot be undone.
                  </>
                ) : bulkConfirm === "banned" ? (
                  <>
                    Ban <strong>{selIds.size} user(s)</strong>? They won't be
                    able to access the platform.
                  </>
                ) : (
                  <>
                    Set <strong>{selIds.size} user(s)</strong> to{" "}
            ...
            <ModalFooter>
              ...
              <AdminBtn
                $variant={
                  bulkConfirm === "delete" || bulkConfirm === "banned"
                    ? "danger"
                    : "primary"
                }
                disabled={bulkWorking}
      ...
      <AdminDataTable
        ...
        renderRow={(o) => (
          <TR key={o.id}>
            <TD>
              <PersonCell>
                <Avatar style={{ width: 30, height: 30, fontSize: "0.7rem" }}>
                  {(o.userName || "?").charAt(0)}
                </Avatar>
                <div>
            ...
            >
              ...
              {o.trackingCode && (
                <div
                  style={{ fontSize: "0.7rem", color: "#06b6d4", marginTop: 2 }}
                >
                  🚚 {o.trackingCode}
                </div>
              )}
            ...
            >
              <StatusDropdown
                value={o.status}
                onChange={(val) =>
                  handleUpdateOrderStatus(o.id, val as Order["status"])
                }
            ...
            >
              <PortalDropdown>
                ...
                <MenuItem
                  onClick={() => {
                    closeAllDropdowns();
                    navigate(`/admin/orders/${o.id}/invoice`);
                  }}
                >
...
const SourceBadge = styled.span<{ $auto?: boolean }>`
  ...
  font-weight: 600;
  background: ${(p) =>
    p.$auto ? "rgba(130,174,70,.15)" : "rgba(59,130,246,.15)"};
...
const CARD_COLS: ColDef[] = [
  ...
  { key: "captured", label: "Captured" },
  {
    ...
    label: "",
    sortable: false,
    thProps: { $center: true, $width: "60px" },
  },
];
...
export const ContactsPage: React.FC = () => {
  ...
  const {
    ...
    error,
    refetch,
  } = useAdminContacts(query);

  const changeStatus = useCallback(
    async (c: Contact, status: Contact["status"]) => {
      try {
        await adminContactsApi.updateStatus(c.id, status);
        dispatch(
          showAdminToast({ message: `Marked as "${status}"`, type: "success" }),
        );
  ...
  const handleDelete = useCallback(
    async (id: string) => {
      try {
        ...
        dispatch(
          showAdminToast({ message: "Message deleted", type: "warning" }),
        );
        refetch();
      } catch (err) {
        dispatch(
  ...
  return (
    <>
      ...
      <AdminDataTable
        ...
        renderRow={(c) => (
          ...
          >
            <TD>
              <PersonName
                style={{ fontWeight: c.status === "new" ? 700 : 500 }}
              >
                {c.name}
              </PersonName>
              <PersonSub>{c.email}</PersonSub>
            </TD>
            <TD
              style={{
                maxWidth: 260,
            ...
            <TD>
              <StatusPill $variant={contactStatusV(c.status) as any}>
                {c.status}
              </StatusPill>
            </TD>
            ...
            >
              <PortalDropdown>
                ...
                <MenuItem onClick={() => changeStatus(c, "replied")}>
                  <CheckCircle size={13} /> Mark Replied
                </MenuItem>
                <MenuItem onClick={() => changeStatus(c, "archived")}>
                  <MoreHorizontal size={13} /> Archive
                </MenuItem>
                <MenuItem $danger onClick={() => handleDelete(c.id)}>
                  <Trash2 size={13} /> Delete
                </MenuItem>
              </PortalDropdown>
            </TD>
      ...
      {selected && (
        <ModalBackdrop onClick={() => setSelected(null)}>
          <ModalBox $width="540px" onClick={(e) => e.stopPropagation()}>
            <ModalHeader>
              <div>
                <div
                  style={{
              ...
              <AdminFlex as="div" $gap="8px">
                <StatusPill $variant={contactStatusV(selected.status) as any}>
                  {selected.status}
                </StatusPill>
                <IconBtn onClick={() => setSelected(null)}>✕</IconBtn>
              </AdminFlex>
            ...
            <ModalBody>
              ...
              <p style={{ fontSize: "0.8rem", color: t.colors.textMuted }}>
                Received: {formatDate(selected.createdAt)}
              </p>
              {selected.repliedAt && (
                <p style={{ fontSize: "0.8rem", color: t.colors.textMuted }}>
                  Replied: {formatDate(selected.repliedAt)}
                </p>
              )}
            </ModalBody>
            <ModalFooter>
              ...
              <AdminBtn
                $variant="primary"
                onClick={() => {
                  changeStatus(selected, "replied");
                  setSelected(null);
                }}
              >
                <Mail size={14} /> Mark Replied
              </AdminBtn>
            </ModalFooter>
          </ModalBox>
        </ModalBackdrop>
...
export const BlogsPage: React.FC = () => {
    ...
    async (action: "published" | "draft" | "delete") => {
      ...
      try {
        if (action === "delete") {
          ...
          dispatch(
            showAdminToast({
              message: `${ids.length} post(s) set to ${action}`,
              type: "success",
            }),
          );
        }
  ...
  const openView = (b: AdminBlogPost) => {
    closeAllDropdowns();
    setViewTarget(b);
    setBlogModal("view");
  };
  const openDelete = (b: AdminBlogPost) => {
    closeAllDropdowns();
    setDeleteTarget(b);
    setBlogModal("delete");
  };
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      dispatch(
        showAdminToast({
          message: `"${deleteTarget.title}" deleted`,
          type: "warning",
        }),
      );
      closeModal();
            ...
            {selIds.size > 0 && (
              <BulkBar>
                ...
                <BulkActionBtn
                  $variant="success"
                  disabled={bulkWorking}
                  onClick={() => setBulkConfirm("published")}
                >
                ...
                <BulkActionBtn
                  $variant="warning"
                  disabled={bulkWorking}
                  onClick={() => setBulkConfirm("draft")}
                >
                  <XCircle size={12} /> Set Draft
                </BulkActionBtn>
                <BulkActionBtn
                  $variant="danger"
                  disabled={bulkWorking}
          ...
          <TR key={b.id}>
            ...
            <TD style={{ padding: "10px 8px 10px 16px" }}>
              {b.cover && b.cover.startsWith("http") ? (
                ...
                >
                  <BookOpen size={18} color={t.colors.textMuted} />
                </div>
              )}
            </TD>
            ...
            >
              <PortalDropdown>
                <MenuItem onClick={() => openView(b)}>
                  <Eye size={13} /> View
                </MenuItem>
                <MenuItem onClick={() => openEdit(b)}>
                  <Edit2 size={13} /> Edit
                </MenuItem>
                <MenuItem $danger onClick={() => openDelete(b)}>
                  <Trash2 size={13} /> Delete
                </MenuItem>
              </PortalDropdown>
            </TD>
              ...
              <FormGrid $cols={2}>
                ...
                <FormGroup>
                  ...
                  <AdminDropdown
                    ...
                    options={BLOG_CATS.map((c) => ({ value: c, label: c }))}
                  />
                </FormGroup>
                <FormGroup>
            ...
            <ModalBody>
              ...
              >
                {bulkConfirm === "delete" ? (
                  <>
                    Are you sure you want to delete{" "}
                    <strong>{selIds.size} post(s)</strong>? This cannot be
                    undone.
                  </>
                ) : (
                  <>
                    Set <strong>{selIds.size} post(s)</strong> to{" "}
                    <strong>{bulkConfirm}</strong>?
                  </>
                )}
              </p>
