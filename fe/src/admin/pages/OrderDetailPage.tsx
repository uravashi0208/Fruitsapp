...
> = {
  pending: {
    ...
    fg: t.colors.warning,
    border: `${t.colors.warning}44`,
  },
  confirmed: {
    bg: t.colors.infoBg,
    fg: t.colors.info,
    border: `${t.colors.info}44`,
  },
  ...
  .step-icon {
    ...
    background: ${(p) =>
      p.$cancelled
        ? t.colors.dangerBg
        : p.$done || p.$active
          ? t.colors.primaryGhost
          : "transparent"};
    box-shadow: ${(p) =>
      p.$active ? `0 0 0 4px ${t.colors.primaryGhost}` : "none"};
    color: ${(p) =>
      p.$cancelled
        ? t.colors.danger
        : p.$done || p.$active
...
const timelineColor = (status: string) =>
  STATUS_COLORS[status]?.fg ?? t.colors.textMuted;
...
export const OrderDetailPage: React.FC = () => {
    ...
    try {
      ...
      if (orderRes.status === "fulfilled" && orderRes.value.success) {
        ...
        if (o.estimatedDelivery)
          setEstDelivery(o.estimatedDelivery.slice(0, 10));
      }
    ...
    try {
      await adminTrackingApi.assign(id, {
        ...
        estimatedDelivery: estDelivery
          ? new Date(estDelivery).toISOString()
          : undefined,
        note: trkNote || undefined,
      });
      dispatch(
        showAdminToast({
          message: "Tracking assigned & customer notified",
          type: "success",
        }),
      );
      setTrkNote("");
      ...
      dispatch(
        showAdminToast({ message: "Failed to assign tracking", type: "error" }),
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleAddEvent = useCallback(async () => {
    ...
    try {
      ...
      dispatch(
        showAdminToast({ message: "Tracking event added", type: "success" }),
      );
      setEvtNote("");
      setEvtLocation("");
      setEvtStatus("");
      ...
      dispatch(
        showAdminToast({ message: "Failed to add event", type: "error" }),
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleSaveAdminNote = useCallback(async () => {
    ...
    try {
      ...
      dispatch(
        showAdminToast({ message: "Admin note saved", type: "success" }),
      );
    } catch {
      dispatch(
        showAdminToast({ message: "Failed to save note", type: "error" }),
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <Page>
      {/* Header */}
      <Header>
        <BackBtn onClick={() => navigate("/admin/orders")}>
          ← Back to Orders
        </BackBtn>
        <TitleBlock>
            ...
            <StepBar>
              {STEPS.map((s, i) => (
                <React.Fragment key={s.key}>
                  ...
                  {i < STEPS.length - 1 && (
                    <StepConnector $done={i < activeStep} />
                  )}
                </React.Fragment>
              ))}
            </StepBar>
      ...
      <Grid>
              ...
              <SummaryRow>
                ...
                <span>
                  {order.shipping > 0 ? (
                    ...
                    <span style={{ color: t.colors.success, fontWeight: 600 }}>
                      Free
                    </span>
                  )}
                </span>
          ...
          <Section $stripe={t.colors.info}>
            ...
            <SectionBody>
              {timeline?.carrierInfo?.trackingUrl && (
                <a
                  href={timeline.carrierInfo.trackingUrl}
                  target="_blank"
              ...
              {tl.length === 0 ? (
                ...
                >
                  No tracking events yet. Assign a tracking code or add events
                  below.
                </div>
              ) : (
                <Timeline>
                    ...
                    return (
                      <TLItem key={ev.id || i}>
                        ...
                        <TLMeta>
                          ...
                          <TLBadge $bg={sc?.bg} $fg={sc?.fg}>
                            {ev.status}
                          </TLBadge>
                          {ev.actor && ev.actor !== "system" && (
                            <TLBadge>by {ev.actor}</TLBadge>
                          )}
                        </TLMeta>
                      </TLItem>
                    );
        ...
        <Col>
            ...
            <SectionBody>
              <InfoGrid>
                ...
                <InfoItem>
                  ...
                  <InfoValue>
                    <StatusTag $s={order.paymentStatus}>
                      {order.paymentStatus}
                    </StatusTag>
                  </InfoValue>
                </InfoItem>
              ...
              {order.address && Object.keys(order.address).length > 0 && (
                <>
                  ...
                  <InfoValue
                    style={{ lineHeight: 1.7, color: t.colors.textSecondary }}
                  >
                    {[
          ...
          <Section $stripe={t.colors.warning}>
            <SectionHead>
              <SectionHeadTitle>⚙️ Update Status</SectionHeadTitle>
            </SectionHead>
            <SectionBody>
              <FormRow>
              ...
              <FormRow>
                ...
                <AdminInput
                  placeholder="e.g. Confirmed by warehouse team"
                  value={statusNote}
          ...
          <Section $stripe={t.colors.info}>
            <SectionHead>
              <SectionHeadTitle>📍 Assign Tracking</SectionHeadTitle>
            </SectionHead>
            <SectionBody>
              {order.trackingCode && (
                <TrackingCodeBox>
                  ...
                  <CopyBtn onClick={() => copyCode(order.trackingCode!)}>
                    {copied ? "✓" : "📋 Copy"}
                  </CopyBtn>
                </TrackingCodeBox>
              )}
              <FormRow>
                ...
                <AdminInput
                  placeholder="VGF-XXXX-XXXXXX"
                  value={trkCode}
                  onChange={(e) => setTrkCode(e.target.value.toUpperCase())}
                />
              </FormRow>
              <FormRow>
                <FormLabel>
              ...
              <FormRow>
                ...
                <AdminInput
                  placeholder="Your order has been handed to the carrier"
                  value={trkNote}
                  onChange={(e) => setTrkNote(e.target.value)}
                />
              </FormRow>
              <BtnRow>
                <AdminBtn
              ...
              <FootNote>
                ✉️ Customer will receive an email with the tracking code when
                you click Assign.
              </FootNote>
            </SectionBody>
          </Section>
          ...
          <Section $stripe="#7c3aed">
            <SectionHead>
              <SectionHeadTitle>➕ Add Tracking Event</SectionHeadTitle>
            </SectionHead>
            <SectionBody>
              <FormRow>
                ...
                <AdminDropdown
                  ...
                  value={evtStatus}
                  onChange={(val) => {
                    setEvtStatus(val as any);
                  }}
                  options={[
                    { value: "", label: "— same as order status —" },
                    { value: "pending", label: "Pending" },
                    { value: "confirmed", label: "Confirmed" },
                    { value: "processing", label: "Processing" },
              ...
              <FormRow>
                ...
                <AdminInput
                  placeholder="e.g. Warsaw Distribution Center"
                  value={evtLocation}
              ...
              <FormRow>
                ...
                <AdminTextarea
                  placeholder="e.g. Package sorted and ready for courier pickup"
                  value={evtNote}
          ...
          <Section>
            <SectionHead>
              <SectionHeadTitle>📝 Admin Note</SectionHeadTitle>
            </SectionHead>
            <SectionBody>
              <FormRow>
