...
const Thumb = styled.button<{ $active: boolean }>`
  ...
  height: 72px;
  border: 2px solid
    ${({ $active }) => ($active ? theme.colors.primary : "#f0f0f0")};
  overflow: hidden;
  cursor: pointer;
  padding: 0;
  background: none;
...
const StarBtn = styled.button<{ $filled: boolean }>`
  ...
  font-size: 30px;
  line-height: 1;
  color: ${({ $filled }) => ($filled ? "#ffc107" : "#dee2e6")};
  transition:
    color 0.15s,
    transform 0.1s;
  &:hover {
...
/* ── ReviewForm ── */
const ReviewForm: React.FC<{ productId: string; onSubmitted: () => void }> = ({
  productId,
  onSubmitted,
}) => {
  ...
  if (success)
    return (
      <FormSuccess>✓ Thank you! Your review has been submitted.</FormSuccess>
    );
  ...
  return (
    <ReviewFormBox>
      <FormTitle>
        ...
        Write a Review
        {isLoggedIn && (
          <GuestLabel>Logged in as {userData?.name || "User"}</GuestLabel>
        )}
      </FormTitle>
      ...
      >
        <Button
          ...
          disabled={submitting}
          style={{ minWidth: 140, opacity: submitting ? 0.7 : 1 }}
        >
      ...
      <ReviewHeader>
        <ReviewerInfo>
          ...
          <div>
            ...
            <ReviewDate style={{ display: "block", marginTop: 2 }}>
              {date}
            </ReviewDate>
          </div>
        </ReviewerInfo>
...
const ReviewsBlock: React.FC<{ productId: string; refreshKey: number }> = ({
  productId,
  refreshKey,
}) => {
  ...
  if (loading)
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {[1, 2, 3].map((i) => (
          <SkeletonBox key={i} style={{ height: 90 }} />
        ))}
      </div>
    );
      ...
      {totalPages > 1 && (
        <PaginationRow>
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <PageBtn key={p} $active={p === page} onClick={() => goTo(p)}>
              {p}
            </PageBtn>
          ))}
          <PageBtn
          ...
          <DetailLayout>
              ...
              <ThumbnailRow as="div">
                ...
                >
                  <img
                    ...
                    onError={(e) => {
                      (e.target as HTMLImageElement).src =
                        "https://placehold.co/72x72/f1f8f1/82ae46?text=.";
                    }}
                  />
                </Thumb>
              </ThumbnailRow>
            ...
            <ProductInfo>
                  ...
                  <QuantityBtn
                    onClick={() =>
                      setQty((q) =>
                        Math.min(
                          q + 1,
                          productInStock ? (product.stock ?? 99) : 1,
                        ),
                      )
                    }
              ...
              <PerksBox>
                ...
                <PerkRow as="div">
                  <CheckCircle size={16} />
                  <span>Certified 100% organic produce</span>
                </PerkRow>
              </PerksBox>
            </ProductInfo>
          </DetailLayout>
