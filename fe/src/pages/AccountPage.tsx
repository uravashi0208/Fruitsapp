...
const AccountPage: React.FC = () => {
      ...
      try {
        ...
        if (ordersRes.success) {
          ...
          const pts = ordersRes.data
            ...
            .reduce(
              (acc: number, o: PlacedOrder) => acc + Math.floor(o.total ?? 0),
              0,
            );
          setPoints(pts);
        }
  ...
  const handleCancel = async (orderId: string) => {
    ...
    try {
      ...
      dispatch(
        showToast({
          message: e.message || "Could not cancel order",
          type: "error",
        }),
      );
    } finally {
          ...
          <Layout>
            ...
            <Content>
              ...
              {tab === "wishlist" && (
                <Card>
                  <CardTitle>
                    <Heart size={18} /> My Wishlist ({wishItems.length})
                  </CardTitle>
                  {wishItems.length === 0 ? (
                    <EmptyMsg>
                      <Heart
                          ...
                          <img
                            src={
                              item.image?.startsWith("http")
                                ? item.image
                                : `${process.env.REACT_APP_API_URL || "http://localhost:4000"}${item.image}`
                            }
                            alt={item.name}
                          />
                          <div>
              ...
              {tab === "loyalty" && (
                <Card>
                  <CardTitle>
                    <Star size={18} /> Loyalty Points
                  </CardTitle>
                  <PointsBadge>
                    <Star size={16} /> {points} Points
                  </PointsBadge>
                  <p
                    style={{
                      fontSize: 14,
                  ...
                  >
                    You earn <strong>1 point per $1</strong> spent on delivered
                    orders. Redeem <strong>100 points = $1 discount</strong> at
                    checkout.
                  </p>
                  <div
                    style={{
                  ...
                  >
                    ...
                    >
                      ...
                      <strong>${(points * 0.01).toFixed(2)}</strong>
                    </div>
                    <div
                      style={{
                        display: "flex",
                    ...
                    >
                      ...
                      <strong>
                        {Math.max(0, 100 - (points % 100))} pts away
                      </strong>
                    </div>
                  </div>
