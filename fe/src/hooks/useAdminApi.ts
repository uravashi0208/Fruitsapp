...
import {
  adminDashboardApi, adminProductsApi, adminOrdersApi,
  adminUsersApi, adminCardsApi, adminContactsApi, adminWishlistApi,
  adminBlogsApi, adminSettingsApi, adminCategoriesApi, adminTestimonialsApi, adminSlidersApi, adminNewsletterApi,
  DashboardStats, AdminProduct, Order, AdminUser, CardDetail, Contact,
  WishlistAdminEntry, WishlistByUser, AdminBlogPost, SiteSettings, AdminCategory, AdminTestimonial, AdminSlider, NewsletterSubscriber,
  ProductQuery, OrderQuery, UserQuery, CardQuery, ContactQuery, BlogQuery,
} from '../api/admin';
...
function useAdminPaged<T>(
  fetchFn: () => Promise<{ success: boolean; data: T[]; pagination: { total: number; totalPages: number; page: number; limit: number } }>,
  deps: unknown[] = []
) {
...
export const useAdminProducts = (query: ProductQuery = {}) =>
  useAdminPaged<AdminProduct>(
    ...
    [query.page, query.limit, query.search, query.category, query.status, query.sortDir]
  );
...
// ── Wishlist ──────────────────────────────────────────────────
export const useAdminWishlist = (query: { search?: string; userId?: string } = {}) =>
  useAdminFetch<{ entries: WishlistAdminEntry[]; byUser: WishlistByUser[]; total: number }>(
    () => adminWishlistApi.list(query),
...
// ── Newsletter ─────────────────────────────────────────────────
export const useAdminNewsletter = (params: { page?: number; limit?: number; status?: string } = {}) => {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      setError(e instanceof ApiError ? e.message : 'Failed to load subscribers');
    } finally {
      setLoading(false);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
